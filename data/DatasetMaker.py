...
class DatasetMaker:
    ...
    def make(self, target_df, output, save):
        """
        Generates and prepares a dataset.
        Can save the intermediary results in a desired file.
        Args:
        -----
            ...
            :output:    str, containing the prefix of the made datsets.
            :save:      str containing the prefix
                        of the files that will store the intermediary results
                        of the generation.
        """
        self._debug("It will take a certain time... \nHave to make {0} matches!".format(
            target_df.shape[0]))
        # Generate
    ...
    def prepare(self, target_df, output):
        """
        ...
        """
        # Prepare
        prepared = self._prepare(target_df)
...
if __name__ == '__main__':
    ...
    # Config
    config = {"stats_type": ['common', 'avg'], "discount": [("week", 0.995)], "surface": corr}

    # DB
    f = 'fusion2/raw_2001_2018.csv'
    df = pd.read_csv(f, index_col=None, header=0, low_memory=False)
    df['date'] = pd.to_datetime(df['date'], format="%Y/%m/%d")

    dm = DatasetMaker(df, config, debug=True)
    ...
    to_prepare = pd.read_csv('tournament_256_20180.csv', index_col=None, header=0, low_memory=False)
    to_prepare['date'] = pd.to_datetime(to_prepare['date'], format="%Y/%m/%d")

    #n_cores = 4
    # df_split = np.array_split(dataset_df, 10)  # Split the dataset
    #pool = multiprocessing.Pool(n_cores)

    #prepare = functools.partial(dm.prepare, output="prepared.csv")

    #generate = functools.partial(dm.generate, output="raw_dataset")
    #pool.map(prepare, df_split)

    #to_prepare = pd.read_csv('raw_dataset0.csv', index_col=None, header=0, low_memory=False)

    dm.prepare(to_prepare, "prepared.csv")
