...
for h in range(int(start_year), int(end_year) + 1):
    ...
    for i in range(0, len(tourney_urls_scrape)):
        if len(tourney_urls_scrape[i]) > 0:
            ...
            spacing1 = ''
            for j in range(0, spacing_count1):
                spacing1 += ' '

            spacing_count2 = 41 - len(tourney_data_scrape[i][2])
            spacing2 = ''
            for j in range(0, spacing_count2):
                spacing2 += ' '

            print(year + '    ' + str(tourney_data_scrape[i][1]) + spacing1 + '    ' +
                  tourney_data_scrape[i][2] + spacing2 + ' ' + str(len(match_data_scrape)))

        filename = "csv/" + "match_scores_" + start_year + "-" + end_year
