...
print('')
print('Collecting match stats data for ' +
      '\x1b[0;32;40m' + str(tourney_count) + '\x1b[0m' + ' tournaments:')
print('')
...
for i in range(int(start_index), tourney_count):
    ...
    if len(match_stats_url_suffixes) > 0:
        ...
        match_stats_data_scrape += asynchronous(match_stats_url_suffixes,
                                                scrape_match_stats, tourney_index, tourney_slug)
        ...
        print(tourney_index + spacing1 + '    ' + tourney_slug +
              spacing2 + '    Match stats URL problems')

    # STEP 3: Output to CSV
    filename = "csv/" + "match_stats_" + year + "_" + start_index
